...
/*   Updated: 2024/06/14 16:18:32 by ting             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parsing(t_lexer **lexer, t_cmd **cmds)
{
    ...
    while (*lexer)
    {
        arr = ft_calloc((cal_arg_count(*lexer) + 1), sizeof(char *));
        cmd = new_cmd(arr);
        while (*lexer && (*lexer)->type != 2)
        {
            if ((*lexer)->type >= 3 && (*lexer)->type <= 6)
            {
                if (check_redirection(*lexer, cmd, arr))
                    return (free_array(arr), free(arr), free(cmd), 1);
                *lexer = (*lexer)->next;
            }
            else
				arr[get_arr_size(arr)] = ft_strdup((*lexer)->str);
            *lexer = (*lexer)->next;
        }
